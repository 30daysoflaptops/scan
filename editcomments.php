...
if( $_REQUEST['action'] == "edit" ) {
	...
	if ($config['comments_mobile_editor'] and $area != "adminpanel") {
...
HTML;
	...
	if( $config['allow_comments_wysiwyg'] < 1 ) {
		...
		if ($config['allow_comments_wysiwyg'] == "1" AND !$comments_mobile_editor) {	
...
HTML;
	...
	$db->query( "UPDATE " . PREFIX . "_{$allowed_areas[$area]['comments_table']} SET {$set_author}text='{$comm_update}', approve='1' WHERE id = '{$id}'" );
