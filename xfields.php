  ...
  case "templatereplacepreview":
    ...
    foreach ($xfields as $value) {
		...
		if ($value[3] == "select") {
			...
			if (is_array($_POST['xfield'][$value[0]]) and count($_POST['xfield'][$value[0]])) {
				$options = explode("\r\n", $value[4]);
				$temp_arr = [];
