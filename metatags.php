...
if( !$user_group[$member_id['user_group']]['admin_meta']  ) {
...
if ($_GET['action'] == "disable") {
	...
	$db->query("INSERT INTO " . USERPREFIX . "_admin_logs (name, date, ip, action, extras) values ('" . $db->safesql($member_id['name']) . "', '{$_TIME}', '{$_IP}', '124', '')");
	$db->query("UPDATE " . PREFIX . "_metatags SET `enabled`='0' WHERE id='{$id}'");
...
if ($_GET['action'] == "enable") {
	...
	$db->query("INSERT INTO " . USERPREFIX . "_admin_logs (name, date, ip, action, extras) values ('" . $db->safesql($member_id['name']) . "', '{$_TIME}', '{$_IP}', '125', '')");
	$db->query("UPDATE " . PREFIX . "_metatags SET `enabled`='1' WHERE id='{$id}'");
  ...
  <div class="panel-heading">
	...
	<div class="heading-elements">
		<div class="form-group has-feedback" style="width:250px;">
			<input name="searchword" dir="auto" type="search" class="form-control" placeholder="{$lang['search_field']}" onchange="document.optionsbar.start_from.value=0;" value="{$searchword}">
...
if ( $all_count_news ) {
	...
	while($row = $db->get_row()) {
		...
		if ($row['enabled']) {
			...
			$status = "<span title=\"{$lang['metatags_off']}\" class=\"text-danger tip\"><b><i class=\"fa fa-exclamation-circle\"></i></b></span>";
        ...
        <div class="btn-group">
          ...
          <ul class="dropdown-menu text-left dropdown-menu-right">
			...
			<li><a href="?mod=metatags&start_from={$start_from}&user_hash={$dle_login_hash}{$urlsearch}&action={$led_action}&id={$row['id']}"><i class="fa fa-eye position-left"></i>{$lang['led_active']}</a></li>
			<li class="divider"></li>
...
HTML;
        ...
        <td><div id=\"title_{$row['id']}\" data-description=\"{$row['description']}\" data-keywords=\"{$row['keywords']}\" data-pagetitle=\"{$row['page_title']}\" data-robots=\"{$row['robots']}\">{$row['title']}</div><textarea dir=\"auto\" id=\"descr_{$row['id']}\" style=\"display:none;\">{$row['page_description']}</textarea></td>
        <td>{$status}</td>
...
<!--
	...
	function addLink() {
		...
		$("body").append("<div id='dlepopup' title='{$lang['add_links_meta']}' style='display:none'><form id='addtags' method='post'><input type='hidden' name='mod' value='metatags'><input type='hidden' name='action' value='add'><input type='hidden' name='user_hash' value='{$dle_login_hash}'><input type='text' dir='auto' name='url' id='dle-promt-url' class='classic' style='width:100%;' value=''placeholder='{$lang['meta_param_1']}'><br><br>{$lang['page_header_1']}<br><input type='text' dir='auto' name='page_title' class='classic' style='width:100%;' value=''><br><br>{$lang['page_header_2']}<br><textarea dir='auto' name='page_description' class='classic' style='width:100%;' rows='3' placeholder='{$lang['page_header_3']}'></textarea><br><br>{$lang['meta_title']}<br><input type='text' dir='auto' name='meta_title' class='classic' style='width:100%;' value=''><br><br>{$lang['meta_descr']}<br><input type='text' dir='auto' name='descr' class='classic' style='width:100%;' value=''><br><br>{$lang['meta_keys']}<br><input type='text' dir='auto' name='keywords' class='classic' style='width:100%;' value='' id='dle-promt-keywords'><br><br>{$lang['meta_robots']}<br><input type='text' dir='auto' name='robots' class='classic' style='width:100%;' value=\"\"></form></div>");
...
$(function(){
		...
		$('.editlink').click(function(){
			...
			$("body").append("<div id='dlepopup' title='{$lang['add_links_meta']}' style='display:none'><form id='edittags' method='post'><input type='hidden' name='id' value='"+urlid+"'><input type='hidden' name='mod' value='metatags'><input type='hidden' name='action' value='edit'><input type='hidden' name='searchword' value='{$searchword}'><input type='hidden' name='start_from' value='{$start_from}'><input type='hidden' name='user_hash' value='{$dle_login_hash}'>{$lang['meta_param_1']}<br><input type='text' dir='auto' name='url' id='dle-promt-url' class='classic' style='width:100%;' value='"+url+"'/><br><br>{$lang['page_header_1']}<br><input type='text' dir='auto' name='page_title' class='classic' style='width:100%;' value='"+page_title+"'><br><br>{$lang['page_header_2']}<br><textarea dir='auto' name='page_description' class='classic' style='width:100%;' rows='3' placeholder='{$lang['page_header_3']}'>"+page_description+"</textarea><br><br>{$lang['meta_title']}<br /><input type='text' dir='auto' name='meta_title' class='classic' style='width:100%;' value='"+meta_title+"'/><br><br>{$lang['meta_descr']}<br /><input type='text' dir='auto' name='descr' class='classic' style='width:100%;' value='"+meta_description+"'><br><br>{$lang['meta_keys']}<br><input type='text' dir='auto' name='keywords' class='classic' style='width:100%;' value='"+meta_keywords+"' id='dle-promt-keywords'><br><br>{$lang['meta_robots']}<br><input type='text' dir='auto' name='robots' class='classic' style='width:100%;' value='"+meta_robots+"'></form></div>");
